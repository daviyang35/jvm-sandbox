...
public class AgentLauncher {
    ...
    private static synchronized ClassLoader loadOrDefineClassLoader(final String namespace,
        ...
        return isAlreadyLoaded(namespace) ?
                retrieveExistClassLoader(namespace) :
                createNewClassLoaderAndCache(namespace, coreJar);
    }

    private static SandboxClassLoader retrieveExistClassLoader(String namespace) {
        return sandboxClassLoaderMap.get(namespace);
    }

    private static SandboxClassLoader createNewClassLoaderAndCache(String namespace, String coreJar) throws MalformedURLException {
        final SandboxClassLoader classLoader = new SandboxClassLoader(namespace, coreJar);
        sandboxClassLoaderMap.put(namespace, classLoader);
        return classLoader;
    }

    private static boolean isAlreadyLoaded(String namespace) {
        return sandboxClassLoaderMap.containsKey(namespace) && null != retrieveExistClassLoader(namespace);
    }
    ...
    public static synchronized void uninstall(final String namespace) throws Throwable {
        final SandboxClassLoader sandboxClassLoader = retrieveExistClassLoader(namespace);
    ...
    private static synchronized InetSocketAddress install(final Map<String, String> featureMap,
        ...
        try {
            ...
            injectJVMSandboxSpy(inst, home);

            // 构造自定义的类加载器，尽量减少Sandbox对现有工程的侵蚀
            final ClassLoader sandboxClassLoader = loadOrDefineClassLoader(namespace,
    ...
    private static void injectJVMSandboxSpy(Instrumentation inst, String home) throws IOException {
        // 将Spy注入到BootstrapClassLoader
        final File jvmSandboxSpyJarPath = new File(getSandboxSpyJarPath(home));
        inst.appendToBootstrapClassLoaderSearch(new JarFile(jvmSandboxSpyJarPath));
    }
